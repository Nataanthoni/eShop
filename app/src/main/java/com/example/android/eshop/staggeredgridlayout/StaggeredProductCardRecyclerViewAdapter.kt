...
import com.example.android.eshop.network.ImageRequester
...
class StaggeredProductCardRecyclerViewAdapter(
    private val productList: List<ProductEntry>?) : RecyclerView.Adapter<StaggeredProductCardViewHolder>() {

    override fun getItemViewType(position: Int): Int {
        return position % DISPLAY_COLUMNS
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): StaggeredProductCardViewHolder {
        var layoutId = R.layout.es_staggered_product_card_first
        if (viewType == 1) {
            layoutId = R.layout.es_staggered_product_card_second
        } else if (viewType == 2) {
            layoutId = R.layout.es_staggered_product_card_third
        }
